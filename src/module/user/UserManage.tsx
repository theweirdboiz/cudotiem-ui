import { ActionDelete, ActionEdit, Button, LabelStatus, Paginate, Table } from '~/components'
...
const UserManage = () => {
  ...
  //                       <time className='text-xs text-gray-400'>{user.createdAt}</time>
  //                     </div>
  //                   </div>
  //                 </td>
  //                 <td>{user.email}</td>
  ...
  //                     <ActionEdit onClick={() => navigator(`/manage/update-user?id=${user.id}`)} />
  //                     <ActionDelete
  ...
  //       </>
  //     )}
  //     {!filteredData || (filteredData.length === 0 && <>Not have any category</>)}
  //   </>
  // )
  return <></>
}
