...
import { deleteCategory, getAllCategories, getCategory } from '~/services/categoryService'
            ...
            {filteredData.length > 0 &&
              filteredData.map((category) => (
                <tr key={category?.id}>
                  ...
                  <td>
                    {category.status === 1 && <LabelStatus status={CategoryStatus.APPROVED} />}
                    {category.status === 2 && <LabelStatus status={CategoryStatus.PENDING} />}
                    {category.status === 3 && <LabelStatus status={CategoryStatus.REJECTED} />}
                  </td>
                  <td>
                    <div className='flex-center gap-x-2.5'>
                      <ActionView />
                      <ActionEdit onClick={() => navigator(`/manage/update-category?id=${category.id}`)} />
                      <ActionDelete onClick={() => handleDeleteData(category.id)} />
                    </div>
                  </td>
                </tr>
              ))}
